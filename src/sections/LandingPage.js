...
const LandingPage = () => {
  ...
  return (
    <Box id="home" sx={{ width: '100%', minHeight: '100vh', background: 'linear-gradient(180deg, #020617,#0f172a, #020617)', color: '#fff' }}>
      {loading ? (
        // Loader first
        <Loader
          containerStyles={{ backgroundColor: '#020617' }}
          barStyles={{ backgroundColor: '#38bdf8' }}
          dataStyles={{ color: '#fff', fontWeight: 'bold' }}
        />
      ) : (
        // LandingPage content after loader
        <Box sx={{ pt: '10px', display: 'flex', flexDirection: { xs: 'column', md: 'row' }, minHeight: '100vh' }}>
          {/* LEFT SIDE */}
          <Box sx={{ flex: 1, position: 'relative', zIndex: 2, display: 'flex', flexDirection: 'column', justifyContent: 'center', px: { xs: 4, md: 12 }, py: { xs: 8, md: 0 } }}>
            <Typography
              variant="h3"
              sx={{
                fontWeight: 800,
            ...
            <Stack direction="row" spacing={2} sx={{ mt: 5, justifyContent: 'center' }}>
              <Button variant="contained" href="https://www.linkedin.com/in/james-patrick-tsung-52b796273/" target="_blank" rel="noopener noreferrer"
                sx={{ background: 'linear-gradient(90deg, #38bdf8, #6366f1)', px: 4, fontWeight: 'bold', '&:hover': { background: 'linear-gradient(90deg, #0ea5e9, #4f46e5)' } }}>
                Connect with me
              </Button>
              <Button variant="outlined" href={process.env.PUBLIC_URL + "/resume/James_Patrick_Tsung_Resume.pdf"} download="James_Patrick_Tsung_Resume.pdf"
                sx={{ borderColor: '#38bdf8', color: '#38bdf8', px: 4, '&:hover': { borderColor: '#0ea5e9', color: '#0ea5e9' } }}>
                Download my Resume
              </Button>
            </Stack>
          </Box>
          ...
          {/* RIGHT SIDE (3D Model) */}
          <Box sx={{ flex: 1, mt: { xs: 6, md: 0 }, display: "flex", justifyContent: "center", alignItems: "center", zIndex: 1 }}>
